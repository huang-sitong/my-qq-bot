...
from .builtin.user_memory import (
    recall_user_memory,
...
def _make_search_documents_tool(document_store: DocumentRepository) -> BaseTool:
    async def _run(
        query: Annotated[str, Field(
...
def _make_memory_tools(memory_store: MemoryRepository) -> list[BaseTool]:
    async def _remember(
