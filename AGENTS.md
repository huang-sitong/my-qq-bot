...
- **工具定位**：纯函数在 `tools/search_chat_history.py`、`tools/user_memory.py`、`skills/tools.py`；`build_tools` 包装为 BaseTool（闭包绑服务 + InjectedState 注入 thread_id/user_id + 异常降级）。ToolNode `handle_tool_errors`：除 `ToolInvocationError`（原样返回逐字段校验信息供 LLM 纠正）外统一降级、按类名记日志（防 Tavily URL 泄漏）。MemoryStore 首次启动自动迁移旧 `user_memories` 表进 `store` 后 DROP。
